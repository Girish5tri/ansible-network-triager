...
def triage(config, repos):
    ...
    if headers:
        ...
        logging.warning("No authentication token provided. Proceeding without authentication token")
    ...
    for org, repo_name in repos:
        ...
        try:
            ...
            if isinstance(e, requests.exceptions.HTTPError):
                ...
                if e.response.status_code == 403:
                    logging.error("This might be due to rate limiting or authentication issues. Check your GitHub token.")
                elif e.response.status_code == 404:
                    logging.error("Repository not found. Check if the repository exists and you have access to it.")
            continue
        ...
        logging.info(f"Found {len(issues[repo_name])} new issues/PRs for {org}/{repo_name}")
